...
export class Pong extends Scene {
	...
	create() {
		...
		let l_paddel = this.add.rectangle(
			Math.round(PADDEL_WIDTH / 2),
			CENTER_Y,
			PADDEL_WIDTH,
			PADDEL_HEIGHT,
			0xffffff,
		);
		let r_paddel = this.add.rectangle(
			RIGHT_PADDEL_AXIS,
			CENTER_Y,
			PADDEL_WIDTH,
			PADDEL_HEIGHT,
		...
		let b = this.add.circle(
			CENTER_X,
			CENTER_Y,
			Math.round(PADDEL_WIDTH / 2.5),
			0xffffff,
		);
	...
	enemyIA() {
		...
		const paddlePostition = this.right_paddel.body?.position ?? {
			x: 0,
			y: 0,
		};
		...
		if (
			...
			else moveImuvableObject(this.right_paddel, PADDEL_SPEED, "y");
		}
	}

	startBall() {
		(this.ball.body as Physics.Arcade.Body).position =
			new Phaser.Math.Vector2(CENTER_X, CENTER_Y);
		let random = Math.random();
		let angle =
			random <= 0.5
				? Phaser.Math.Between(-45, 45)
				: Phaser.Math.Between(135, 225);
