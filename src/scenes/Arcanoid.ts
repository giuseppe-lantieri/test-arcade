...
import Levels from "./../levels/arcanoid.json";
...
export class Arcanoid extends Scene {
    ...
    create() {
        ...
        this.ballText = this.add
            .text(CENTER_X, CENTER_Y / 10, `BALL: ${this.ballRemaing}`, {
                fontFamily: "Arial Black",
                fontSize: 24,
                color: "#ffffff",
                stroke: "#000000",
        ...
        this.physics.add.collider(
            ...
            this.paddel,
            this
                .reverseBall as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback,
            undefined,
            this,
        );
        ...
        if (this.input.keyboard) {
            ...
            this.keys.esc = this.input.keyboard.addKey(
                Phaser.Input.Keyboard.KeyCodes.ESC,
            );
            this.keys.q = this.input.keyboard.addKey(
        ...
        this.physics.add.collider(
            this.ball,
            this.blocks,
            this.destroyIt as Phaser.Types.Physics.Arcade.ArcadePhysicsCallback,
            undefined,
            this,
        );
    ...
    pauseFunction() {
        ...
        if (this.pause) {
            ...
            (this.ball.body as Physics.Arcade.Body).setVelocity(
                this.prexVelocity?.x,
                this.prexVelocity?.y,
            );
            this.pauseText.setVisible(false);
    ...
    startBall() {
        (this.ball.body as Physics.Arcade.Body).position =
            new Phaser.Math.Vector2(this.paddel.body?.position.x, BALL_AXIS);
    ...
    reverseBall(
        ...
        let angle =
            Math.round((distanceFromCenter * 90) / (PADDEL_HEIGHT / 2)) - 90;
    ...
    destroyIt(
        ...
        if (this.r_blocks == 0) {
            this.level++;
            if (this.level < Levels.length) this.createLevel();
    ...
    worldboundsCheck(
        ...
        if (down) {
            ...
            if (this.ballRemaing > 0) this.startBall();
